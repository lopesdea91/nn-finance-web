...
import { pageMethods } from '@/@core/presentation/views/finance.tag/FinanceTagListPage.methods'
...
export const SearchTag = () => {
  ...
  const cookie = financeTagCookie.get()
  ...
  async function searchForm() {
    ...
    financeTagCookie.set(values)
    await pageMethods.getItems(values)
  }
  async function searchReset() {
    const values = financeTagCookie.reset()
    resetValues(values)
    await pageMethods.getItems(values)
  }
  async function searchTrashed() {
    const values = financeTagCookie.reset(
      !!financeTagCookie.get()?.deleted
        ? {}
        : { deleted: 1 }
    )
    await pageMethods.getItems(values)
  ...
  function add() {
    observer.publish(redirectObserver('/finance/tag/new'))
  }
